...
class Account extends MY_Controller {
	...
	public function do_suscription() {
		...
		$email = json_decode( $post )->data;

		$result = new stdClass();

		$account = $this->account_model->get_account_by_email($email);
		...
		if ( isset($account) ) {
			...
			$result->status = "SUSCRITED";
			$sign_up_form = array();
			$sign_up_form['userFirstName'] = "ghostdata";
			$sign_up_form['userLastName'] = "ghostdata";
			$sign_up_form['userEmail'] = $email;
			$sign_up_form['termsAndConditions'] = "ghostdata";
			$sign_up_form['userPassword'] = "ghostdata";

			$insert_id = $this->account_model->insert_account($sign_up_form, true, true);
